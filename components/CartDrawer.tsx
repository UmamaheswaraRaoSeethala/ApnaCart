            ...
            {cartType && (
              <div className="p-4 bg-gradient-to-r from-green-100 to-blue-100 border-b border-green-200">
                ...
                <div className="flex justify-between text-xs text-gray-600 mt-1">
                  ...
                  <span>{remainingWeightString}kg remaining</span>
                </div>
              </div>
            )}
                ...
                <div className="text-center py-12">
                  <div className="mb-6">
                    ...
                    <p className="text-gray-500 mb-4">
                      {cartType 
                        ? `Select vegetables to fill your ${cartType === 'small' ? 'Small Cart (4.5kg)' : 'Family Cart (7kg)'}`
                        : 'Choose a cart type first, then add fresh vegetables!'
                      }
                    </p>
                    {cartType && (
                      <div className="inline-flex items-center px-4 py-2 bg-green-50 border border-green-200 rounded-lg text-sm text-green-700">
                        ...
                        Target: {cartType === 'small' ? '4.5kg' : '7kg'}
                      </div>
                    )}
                  </div>
                </div>
            ...
            {items.length > 0 && (
              <div className="border-t border-gray-200 p-4 bg-gray-50">
                ...
                {isCartFull ? (
                  ...
                  >
                    <p className="text-sm text-blue-800 font-medium">
                      📦 Add {formatTotalWeight(remainingWeight)} more to complete your {cartType === 'small' ? 'Small Cart' : 'Family Cart'}
                    </p>
                    ...
                    <div className="flex justify-between text-xs text-blue-600 mt-1">
                      <span>{Math.round((totalWeight / cartLimit) * 100)}% filled</span>
                      <span>{formatTotalWeight(remainingWeight)} to go</span>
                    </div>
                  </motion.div>
                )}
                ...
                <div className="flex space-x-3">
                  ...
                  {/* WhatsApp Order Button - Only show when cart is full */}
                  {isCartFull ? (
                    <button
                      onClick={handleWhatsAppOrder}
                      className="flex-1 px-4 py-2 bg-green-500 text-white rounded-lg hover:bg-green-600 transition-colors flex items-center justify-center space-x-2 shadow-lg"
                    >
                    ...
                    <div className="flex-1 relative">
                      <button
                        disabled
                        className="w-full px-4 py-2 bg-gray-300 text-gray-500 rounded-lg cursor-not-allowed flex items-center justify-center space-x-2"
                        title={`Fill your basket to place an order. Need ${formatTotalWeight(remainingWeight)} more.`}
                      >
                        <MessageCircle className="w-4 h-4" />
                        <span>Fill Basket to Order</span>
                      </button>
                      
                      {/* Progress indicator */}
                      <div className="mt-2 text-center">
                        <span className="text-xs text-gray-500">
                          {formatTotalWeight(remainingWeight)} more needed
                        </span>
                      </div>
                    </div>
                  )}
